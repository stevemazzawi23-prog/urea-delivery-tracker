...
export default function EditClientScreen() {
  ...
  const loadClient = async () => {
    ...
    if (found) {
      ...
      setPortalCode(found.portalCode || "");
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (!clientId) return;
    try {
      await updateClient(clientId, { name: name.trim(), company: company.trim(), phone: phone.trim(), email: email.trim(), address: address.trim(), notes: notes.trim(), portalCode: portalCode.trim().toUpperCase() });
      if (Platform.OS !== "web") Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      router.back();
    } catch (error) { Alert.alert("Erreur", "Impossible de mettre a jour le client."); }
  };

  if (!client) {
    return (<ScreenContainer><View style={styles.centered}><ActivityIndicator size="large" color={colors.primary} /></View></ScreenContainer>);
  }
      ...
      <View style={styles.container}>
        ...
        <ScrollView style={styles.form} keyboardShouldPersistTaps="handled">
          <View style={[styles.fieldGroup, styles.portalCodeBox, { backgroundColor: colors.surface, borderColor: colors.primary }]}>
            ...
            <Text style={[styles.hint, { color: colors.muted }]}>Ce code se trouve dans la liste des clients du portail admin SP Logistix.</Text>
          </View>
          <View style={styles.fieldGroup}>
            <Text style={[styles.label, { color: colors.muted }]}>Nom *</Text>
            <TextInput style={inputStyle} placeholder="Nom du client" placeholderTextColor={colors.muted} value={name} onChangeText={setName} returnKeyType="next" />
          </View>
          <View style={styles.fieldGroup}>
