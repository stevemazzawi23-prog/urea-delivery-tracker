...
// Equipment table - stores equipment definitions
export const equipment = mysqlTable("equipment", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(), // Driver/user who owns this equipment
  name: varchar("name", { length: 255 }).notNull(),
  capacity: int("capacity"), // Optional capacity in liters
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
// Client Equipment junction table - links clients to equipment
export const clientEquipment = mysqlTable("clientEquipment", {
  id: int("id").autoincrement().primaryKey(),
  clientId: int("clientId").notNull(),
